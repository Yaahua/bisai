...
**When updating an existing skill after a real execution failure, do not jump straight into wording changes.** First capture four things in concrete terms: the observed failure, the root cause, the rule that should have prevented it, and the smallest reusable guardrail that would stop recurrence. Use that analysis to decide whether the fix belongs in `SKILL.md`, a bundled `reference`, a `script`, or a `template`.

For workflow-heavy skills, treat **priority resolution**, **stage gates**, and **resume safety** as first-class design concerns rather than optional notes. If an agent could plausibly be misled by inherited summaries, previous plans, or partially completed artifacts, the skill should explicitly say how to resolve that conflict before any action begins.
...
#### Encode Guardrails for Failure-Prone Workflows

If a skill governs a multi-stage process that can fail through ordering mistakes, ambiguity, or interrupted execution, encode the guardrails directly in the skill instead of leaving them implicit.

Useful patterns include:

- **Priority rules**: State which source wins when user instructions, current skill rules, existing artifacts, inherited summaries, or prior plans conflict.
- **Stage gates**: For each stage, specify entry conditions, completion conditions, and forbidden actions before the next stage may begin.
- **Resume protocol**: After interruption or context compression, require the agent to restate the current stage, current work unit, prerequisite files, and prohibited next actions before resuming work.
- **Refusal conditions**: For downstream or dependent skills, state when the skill must refuse to start because prerequisites are incomplete.

When a failure has already happened in real usage, prefer explicit guardrails over softer reminders. A sentence like "review after initial work" is often too weak; a gate such as "do not create audit files until all batch review files exist" is much harder to misread.
...
2. Capture the exact struggle, failure, ambiguity, or inefficiency
3. Write a short root-cause note: what happened, what source or wording misled execution, and what should have prevented it
4. Decide the correct fix type: wording clarification, priority rule, gate condition, resume checklist, refusal rule, validation step, or supporting resource
5. Implement the smallest reusable fix in `SKILL.md` and/or bundled resources
6. Test the updated skill on a representative scenario, especially the one that previously failed
7. Re-run validation and iterate again if the failure mode is still possible
