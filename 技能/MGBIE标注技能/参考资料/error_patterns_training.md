...
**正确做法**：只有在短语同时包含稳定品种名、材料编号或可复核专名时，才保留为 VAR；若只是来源性说明，应删除该实体或保留为普通文本，不强行实体化。

> 判断标准：`accession` 必须能够被唯一指认。像 "CAV 1979" 这类带稳定编号的材料可保留；仅有地域或泛化来源描述时默认不标为 VAR。

### 2.9 材料分组词不可过度提升为作物整体性状

**错误模式**：看到 `salt-sensitive accessions`、`salt-tolerant accessions` 就直接补成 `HAS(CROP, TRT)`，把材料分组误当成作物整体固有性状。

**正确做法**：先判断性状究竟修饰的是作物整体，还是某一批材料/群体。若原文真正修饰的是 accession 或 material group，则不要机械补 `CROP -> TRT` 的 `HAS` 关系。

> 可以保留 `stress -> trait` 的 `AFF`，但不能因为句中同时出现作物名，就自动把该性状提升为作物整体属性。

### 2.10 重复作物名的单复数竞争边界

**错误模式**：同一句或前后句同时出现 `soybean` / `soybeans` 时，在同一偏移保留两个竞争性实体，或借用前一句的偏移去支撑后一句关系。

**正确做法**：每个实体必须绑定到原文中的真实 occurrence；若前后句分别出现单复数形式，应分别按真实位置建实体，禁止在同一偏移制造单复数双实体。

> 删除错误实体后，还要同步检查所有依赖它的 `HAS/AFF/CON` 等关系是否仍然指向现存实体。
...
**正确做法**：关系必须绑定到**真实承担语义的那一次出现**。若修正了 head/tail 文本所指的 occurrence，必须同步更新 `head_start/head_end/tail_start/tail_end` 与对应类型字段。

> 典型风险包括：后文因果分句借用前文基因位置、后一句单数作物名借用前一句复数作物名偏移。

### 3.6 并列实体漏标常与并列关系漏标联动

**错误模式**：并列结构中只补一部分 `TRT` 实体，或补了实体却遗漏与同一 `GENE/ABS/MRK/QTL` 相连的成组关系。

**正确做法**：发现并列结构后，要按“实体组 + 关系组”成对检查。例如补齐 `superoxide dismutase`、`peroxidase`、`catalase activities` 三个 `TRT` 后，也要同步补齐对应的三条 `AFF` 关系。
...
在进入 full revision 之前，必须先为**当前块**生成 `chunk_XXX_issue_inventory.md`，至少列出三类内容：
1. 已确认问题
2. 潜在问题
3. 关联问题 / 需要同步检查的字段

> 目的不是只修显性错误，而是防止遗漏由同一错误引起的边界、关系锚点、并列实体、重复出现实体与同句遗漏关系问题。

**最低检查项**：
- 实体文本是否与 `text[start:end]` 完全一致
- 关系中的 head/tail span 是否与实体 span 同步
- 同一实体在同句多次出现时是否选错 occurrence
- 主问题修复后是否遗漏并列实体、同位实体或括号内编号实体
- 删除实体后是否同步删除或改写依赖该实体的关系

### 4.5 启动与恢复的防呆要求

**错误模式**：任务启动后直接展开多个块，或在恢复时顺手预读后续块，导致上下文污染和执行顺序失控。

**正确做法**：
1. 启动时先记录一次切分规则与工作区范围，随后不再反复读取源仓库
2. 明确当前处理区间与 current chunk
3. 任一时刻只允许打开一个 current chunk；当前块未完成前不得进入下一块
4. 如中途中断，恢复时只重读当前块的 JSON、说明文档、审查文档与问题清单

### 4.6 技术校验的最小闭环

每完成一个 chunk 的修订，至少执行以下技术校验：
1. 所有实体满足 `text[start:end] == entity['text']`
2. 所有关系锚点字段与对应实体边界一致
3. 所有关系引用的 head/tail 实体都真实存在
4. 无原文不存在的幻觉实体
5. 若实体数异常下降，回看该块 issue inventory 与 audit，避免过修

### 4.7 作用域与批次边界防呆

**错误模式**：在用户只要求处理 `chunk_011 ~ chunk_020` 时，未经确认就把补齐、同步或上传范围扩展到 `chunk_001 ~ chunk_010` 的错误清单或其他历史材料。

**正确做法**：
1. 启动时先把用户指定区间写入本地工作记录
2. 所有“补齐文档”“统一上传”“仓库同步”动作都只能覆盖该区间
3. 若发现前序批次也存在缺口，只登记为候选事项，不自动执行
...
| chunk_011~020 | 流程错误 | 初标阶段曾一次性展开多块信息整理，破坏“当前块锁定”原则 | 一次只处理 1 个 chunk；当前块说明文档写完后再进入下一块 | 本批次执行复盘 |
| chunk_011~020 | 流程错误 | 审查阶段发现个别块缺少初标说明文档 | 审查前先核对 `chunk_XXX_review.md` 是否存在，不存在先补齐 | 本批次执行复盘 |
| chunk_011~020 | 技术错误 | 修复时容易只改显性错误，遗漏边界、关系锚点与并列实体联动修改 | 先写 `issue_inventory`，再按“已确认/潜在/关联问题”逐项修复 | 本批次执行复盘 |
| chunk_011 #6 | 技术错误 | 两条 `AFF` 关系把后文真正起效的 `SbMYBHv33` 错绑到前文第一次出现的位置 | 关系必须绑定到真实承担语义的 occurrence，并同步全部 anchor 字段 | `chunk_011_issue_inventory.md` |
| chunk_012 #5 | 语义错误 | 将 `salt-sensitive`、`salt-tolerant` 这类 accession 分组词直接上挂到 `foxtail millet` | 区分材料分组与作物整体性状，不机械补 `HAS(CROP, TRT)` | `chunk_012_issue_inventory.md` |
| chunk_012 #6 | 技术错误 | 并列酶活性实体漏标，连带遗漏 3 条 `FtNAC31 -> TRT` 的 `AFF` 关系 | 并列实体补齐时，必须同步补齐成组关系 | `chunk_012_issue_inventory.md` |
| chunk_013 #4 | 语义错误 | 将 `Israeli accession` 这类来源性描述误当成稳定 `VAR` | 仅在具备可复核专名或编号时才保留为 `VAR` | `chunk_013_issue_inventory.md` |
| chunk_013 #5 | 技术错误 | 同偏移保留 `soybean` 与 `soybeans` 竞争实体，并让 `HAS` 借错单数锚点 | 删除竞争实体，并将关系改绑到真实 occurrence | `chunk_013_issue_inventory.md` |
| chunk_011~020 | 技术错误 | 同一文本多次出现时容易选错 occurrence，导致 start/end 漂移 | 选取最接近原位置的 occurrence，并同步关系锚点 | 本批次执行复盘 |
| chunk_011~020 | 技术错误 | 删除或改写实体后，相关关系字段可能残留旧锚点 | 修复后必须做关系存在性与锚点一致性校验 | 本批次执行复盘 |
| chunk_011~020 | 启动防呆 | 任务启动时若不先记录切分规则，后续容易反复回读源仓库 | 启动阶段先记录切分方式与工作范围，后续按本地记录执行 | 本批次执行复盘 |
