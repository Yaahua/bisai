...
- **依据**: 依据 `MGBIE审查技能/SKILL.md` 的拆分审查原则和 `breeding_knowledge_base.md` 中关于 CROP vs VAR 的辨析。
...
本次审查发现，原始标注在复合实体的拆分、部分实体的精确分类以及关系构建方面存在系统性问题。主要技能 `MGBIE标注技能` 在处理长术语和上下文关联时表现不足。建议在后续修正流程中，重点关注并应用本次审查提出的拆分和分类逻辑。
